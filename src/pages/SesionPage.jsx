  ...
  const handleGoogleLogin = () => {
    setTimeout(() => {
      window.location.href =
        "http://localhost:3001/api/auth-routes/auth/google";
    }, 2000);
  };
    ...
    <>
      <section className="border-red-500 bg-gray-200 min-h-screen flex items-center justify-center">
        <div
          className={
            !authState.isLoading
              ? "bg-gray-100 p-5 flex rounded-2xl shadow-lg max-w-3xl"
              : "bg-gray-100 bg-opacity-30 p-5 flex rounded-2xl shadow-lg max-w-3xl"
          }
        >
          {authState.isLoading && (
            <div className="w-[23rem] absolute ">
              ...
              {authState.message && (
                <div>
                  <p className="text-red-600 text-xs mt-1 w-80">
                    <i className="mr-1 fa-solid fa-circle-exclamation"></i>{" "}
                    {authState.error}
                  </p>
                </div>
              )}
